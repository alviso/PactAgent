...
var StatisticWidgets = function() {
    ...
    var _sparklinesWidget = function(element, chartType, qty, chartHeight, interpolation, duration, interval, color) {
        ...
        if(element) {
            ...
            setInterval(function() {
                ...
                if ( element == "#sparklines_color_success" ) data.push( _readPIREvents("42223:40724") * 5 + 0)
                else if ( element == "#sparklines_color_success1" ) data.push( _readPIREvents("42223:47712") * 5 + 0)
                else data.push( _readPIREvents("49906:3297") * 5 + 0)
                //console.log(pirData.size)
                // pop the old data point off the front
                data.shift();

                update();

                let className = "card bg-success-400 has-bg-image"
                for (i in data) {
                    if (data[i] > 0) {
                        className = "card bg-danger-400 has-bg-image"
                        break
                    }
                }
                document.getElementById(element.substring(1)).parentElement.className = className

            }, interval);

            let pirData = new Map()

            var _readPIREvents = function(pir) {
                let url = "https://indoors-api.alertjack.com/v1/pir-state-events?detectorId="+pir+"&access_token=PGnz3AsW6buwbzCFV8DR2gPmibp8hW"
                $.ajax({
                    dataType: "json",
                    url: url,
                    async: false,
                    success: function(data) {
                        data.forEach(e => {
                            pirData.set(e.ts, e.stateCode)
                        })
                        if (pirData.size > 0) {
                            pirData.forEach((value, key) => {
                                if (key < ( Date.now() - 10 * 1000))  {
                                    pirData.delete(key)
                                }
                            })
                        }
                ...
                let retValue = 0
                if (pirData.size > 4) retValue = 4
                else retValue = pirData.size
                return retValue
            };
    ...
    return {
        init: function() {
            ...
            _sparklinesWidget("#sparklines_color_success", "area", 20, 50, "basis", 750, 2000, "rgba(255,255,255,0.75)");
            _sparklinesWidget("#sparklines_color_success1", "area", 20, 50, "basis", 750, 2000, "rgba(255,255,255,0.75)");
            _sparklinesWidget("#sparklines_color_danger", "area", 20, 50, "basis", 750, 2000, "rgba(255,255,255,0.75)");
