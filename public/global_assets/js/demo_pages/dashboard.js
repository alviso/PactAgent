...
var Dashboard = function () {
    ...
    var _DailyRevenueLineChart = function(element, height, url) {
        if (typeof d3 == 'undefined') {
            console.warn('Warning - d3.min.js is not loaded.');
            return;
        }
            ...
            var tooltip = d3.tip()
                ...
                .html(function (d) {
                    return '<ul class="list-unstyled mb-1">' +
                        ...
                        '<li>' + 'Cranks: &nbsp;' + '<span class="font-weight-semibold float-right">' + d.alpha + '</span>' + '</li>' //+
    ...
    var _BarChart = function(element, barQty, height, animate, easing, duration, delay, color, tooltip) {
        ...
        if($(element).length > 0) {
            ...
            d3.json('/actions/nodestats', function (error, bardata) {
                ...
                // Show and hide
                if (tooltip == 'hours' || tooltip == 'goal' || tooltip == 'members' || tooltip == 'nodes') {
                    bars.call(tip)
                ...
                // Online nodes tooltip content
                if (tooltip == 'nodes') {
                    tip.html(function (d, i) {
                        return '<div class="text-center">' +
                            '<h6 class="m-0">' + d + '</h6>' +
                            '<span class="font-size-sm">nodes</span>' +
                            '<div class="font-size-sm">' + (23 - i) + ' days ago' + '</div>' +
                            '</div>'
                    });
                }
    ...
    return {
        ...
        initCharts: function() {
            ...
            _BarChart('#daily-nodes', 24, 115, true, 'elastic', 1200, 50, 'rgba(255,255,255,0.5)', 'nodes');

            _DailyRevenueLineChart('#daily-cranks', 115, '/actions/crankstats');

            _DailyRevenueLineChart('#crkk-price', 215, '/actions/crankkprice');

            const url = 'https://reporter.crankk.io/balances?coin=free.crankk01&address='+$("#address").text()
